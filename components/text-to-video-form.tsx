...
import { Loader2, Sparkles, Download, AlertCircle, Play } from "lucide-react"
    ...
    <>
      <LoginDialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open) {
            // Check auth status after dialog closes
            checkAuthAndFetchCredits()
          }
        }}
        redirectTo="/generate"
          ...
          <Card className="p-4 sm:p-6 bg-card border-border">
            <div className="space-y-4 sm:space-y-6">
              <div className="space-y-2">
                ...
                <Textarea
                  id="prompt"
                  placeholder="A serene ocean sunset with waves crashing on the shore, golden hour lighting, cinematic..."
                  value={prompt}
                  onChange={(e) => setPrompt(e.target.value)}
                  className="min-h-24 sm:min-h-32 resize-none text-sm sm:text-base"
                  disabled={isGenerating}
                />
                <p className="text-xs sm:text-sm text-muted-foreground">Be as detailed as possible for best results.</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={duration} onValueChange={setDuration} disabled={isGenerating}>
                    <SelectTrigger id="duration" className="text-sm sm:text-base">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  <Label htmlFor="aspect-ratio" className="text-sm sm:text-base">
                    Aspect Ratio
                  </Label>
                  <Select value={aspectRatio} onValueChange={setAspectRatio} disabled={isGenerating}>
                    <SelectTrigger id="aspect-ratio" className="text-sm sm:text-base">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
