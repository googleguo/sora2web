...
export async function sendEmail(to: string, subject: string, html: string) {
  ...
  //   from: 'VideoForge AI <noreply@videoforge.ai>',
  //   to,
  //   subject,
  //   html,
  // })
...
export async function sendPurchaseConfirmation(email: string, planName: string, credits: number, amount: number) {
  const subject = "Thank you for your purchase - VideoForge AI"
  const html = `
    <h1>Thank you for your purchase!</h1>
    <p>Your ${planName} plan has been activated.</p>
    <p><strong>Credits added:</strong> ${credits}</p>
    <p><strong>Amount paid:</strong> $${amount}</p>
    <p>Start creating amazing AI videos at <a href="${process.env.NEXT_PUBLIC_APP_URL}">VideoForge AI</a></p>
  `
  await sendEmail(email, subject, html)
}

export async function sendSubscriptionConfirmation(email: string, planName: string, period: string) {
  const subject = "Subscription Activated - VideoForge AI"
  const html = `
    <h1>Welcome to VideoForge AI!</h1>
    <p>Your ${planName} ${period} subscription is now active.</p>
    <p>Your credits have been added to your account.</p>
    <p>Start creating at <a href="${process.env.NEXT_PUBLIC_APP_URL}">VideoForge AI</a></p>
  `
  await sendEmail(email, subject, html)
}

export async function sendCancellationEmail(email: string, planName: string, expiresAt: string) {
  const subject = "Subscription Cancelled - VideoForge AI"
  const html = `
